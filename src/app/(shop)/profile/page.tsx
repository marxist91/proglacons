...
export default function ProfilePage() {
  ...
  const handleChangePassword = async () => {
    ...
    try {
      ...
      console.error('Erreur:', err);
      setSaveMessage({ type: 'error', text: err.message || 'Erreur lors du changement de mot de passe' });
    } finally {
      setIsSaving(false);
    }
  };
  ...
  if (!isAuthenticated) {
    return (
      <div className={`pt-32 pb-24 min-h-screen ${isDark ? 'bg-slate-900' : ''}`}>
        <div className="max-w-md mx-auto px-4">
          <div className={`rounded-[2rem] p-8 shadow-lg border text-center ${
            isDark 
              ? 'bg-slate-800 border-slate-700'
              : 'bg-white border-slate-100'
          }`}>
            <div className="w-20 h-20 bg-gradient-to-br from-cyan-100 to-sky-100 rounded-full flex items-center justify-center mx-auto mb-6">
              <User className="w-10 h-10 text-cyan-600" />
            </div>
            <h1 className={`text-2xl font-black mb-2 ${isDark ? 'text-white' : 'text-[#1E3A8A]'}`}>
              Accédez à votre profil
            </h1>
            <p className={`mb-6 ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>
              Connectez-vous pour voir vos commandes, favoris et gérer votre compte.
            </p>
            <Link
              href="/auth?redirect=/profile"
              className="inline-block w-full bg-gradient-to-r from-cyan-500 to-sky-500 text-white font-bold py-4 px-8 rounded-xl hover:from-cyan-600 hover:to-sky-600 transition-all shadow-lg shadow-cyan-500/25"
            >
              Se connecter
            </Link>
            <p className={`text-sm mt-4 ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>
              Pas encore de compte ?{' '}
              <Link href="/auth?redirect=/profile" className="text-cyan-600 font-semibold hover:underline">
                S'inscrire
              </Link>
            </p>
          </div>
          ...
          <div className="lg:col-span-1">
            <div className={`rounded-[2rem] p-8 shadow-lg border sticky top-32 ${
              ...
              <div className="text-center mb-6">
                <div className="w-20 h-20 bg-gradient-to-br from-[#00ADEF] to-[#1E3A8A] rounded-full flex items-center justify-center mx-auto mb-4 text-white text-2xl font-black">
                ...
                {isEditing ? (
                  <input
                    ...
                    className={`w-full text-center text-xl font-black rounded-xl px-3 py-2 border outline-none focus:ring-2 focus:ring-[#00ADEF] ${
                      isDark 
                        ? 'bg-slate-700 border-slate-600 text-white'
                        : 'bg-slate-50 border-slate-200 text-[#1E3A8A]'
                    }`}
                    placeholder="Votre nom"
                  />
                ) : (
                  <h2 className={`text-xl font-black ${isDark ? 'text-white' : 'text-[#1E3A8A]'}`}>
                    {displayName}
                  </h2>
                )}
              ...
              {/* Infos / Edition */}
              <div className={`space-y-4 mb-6 border-t pt-6 ${isDark ? 'border-slate-700' : 'border-slate-100'}`}>
                ...
                <div className={`flex items-center gap-3 p-3 rounded-xl ${isDark ? 'bg-slate-700/50' : 'bg-slate-50'}`}>
                  ...
                  <div className="flex-1 min-w-0">
                    <p className={`text-[10px] font-bold uppercase tracking-wider ${isDark ? 'text-slate-500' : 'text-slate-400'}`}>Email</p>
                    <p className={`text-sm font-medium truncate ${isDark ? 'text-slate-300' : 'text-slate-700'}`}>{displayEmail || '-'}</p>
                  </div>
                </div>

                {/* Téléphone */}
                <div className={`flex items-center gap-3 p-3 rounded-xl ${isDark ? 'bg-slate-700/50' : 'bg-slate-50'}`}>
                  <Phone size={18} className="text-[#00ADEF]" />
                  <div className="flex-1 min-w-0">
                    <p className={`text-[10px] font-bold uppercase tracking-wider ${isDark ? 'text-slate-500' : 'text-slate-400'}`}>Téléphone</p>
                    {isEditing ? (
                      <input
                        type="tel"
                ...
                {/* Quartier */}
                <div className={`flex items-center gap-3 p-3 rounded-xl ${isDark ? 'bg-slate-700/50' : 'bg-slate-50'}`}>
                  <MapPin size={18} className="text-[#00ADEF]" />
                  <div className="flex-1 min-w-0">
                    <p className={`text-[10px] font-bold uppercase tracking-wider ${isDark ? 'text-slate-500' : 'text-slate-400'}`}>Quartier</p>
                    {isEditing ? (
                      <input
                        type="text"
